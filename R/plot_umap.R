...
#'          (default: TRUE).
#' @param n_perm integer; if pca is TRUE, this is the number of times that the 
#'          rows of the expression matrix should be permuted (default: 10). 
...
#'          (default: square root of the number of samples).
...
#'          Passed to stringr::str_wrap as width argument (default: 20).
...
#' @examples
#' data(srm_msnset)
#' plot_umap(msnset, phenotype = "subject.type", show_ellipse = F)
#' plot_umap(msnset, phenotype = "subject.type", show_ellipse = T)
#' plot_umap(msnset, phenotype = "subject.type", pca = F, show_ellipse = T)
#' plot_umap(msnset)

plot_umap <- function(eset, phenotype = NULL, standardize = F, 
                      pca = TRUE, n_perm = 10,
